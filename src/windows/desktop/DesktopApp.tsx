...
import { fetchPlayerCard, getActiveGame, getServerStatus } from '@/services/riotApiClient';
...
export function DesktopApp() {
  ...
  const sortedMetaComps = useMemo(() => {
    return META_COMPS.map((comp, index) => {
      ...
      return { comp, tier }; // end of map
    });
  }, []);

  // Comp filters state
  const [compTierFilter, setCompTierFilter] = useState('All');
  ...
  useEffect(() => {
    ...
    getServerStatus(region)
      .then((data) => {
        ...
        const maintenances = (data as { maintenances?: unknown[] }).maintenances;
        const hasIssues =
          (incidents?.length ?? 0) > 0 || (maintenances?.length ?? 0) > 0;
...
className="w-8 h-8 rounded-lg flex items-center justify-center text-white hover:bg-[#1a1a1a] hover:scale-105 active:scale-95 focus-visible:ring-2 focus-visible:ring-[#35c3e7] transition-all duration-200"
      ...
      <div className="flex min-h-0 flex-1 overflow-hidden">
        ...
        {/* Page Content */}
        <div key={activePage} className={`min-h-0 min-w-0 flex-1 overflow-y-auto bg-[#0d0d0d] custom-scrollbar ${
        ...
        <DesktopRightRail serverStatus={serverStatus} />
      </div>
      </div>
    </ThemeProvider>
  );
