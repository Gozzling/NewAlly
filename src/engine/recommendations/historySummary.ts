...
function buildTraitThresholdHistory(
  slice: HistoryRow[],
  championMap: Map<string, (typeof BUNDLED_UNITS)[number]>,
  traitMap: Map<string, (typeof BUNDLED_SYNERGIES)[number]>,
  ...
  for (const m of slice) {
    ...
    const p = m.placement;
    if (p < 1) continue;
    const counts = traitCountsFromUnitNames(units, championMap);
    const isTop4 = p <= 4;
...
function aggregateHistoryRows(
  ...
  // Build maps for O(1) lookups during aggregation
  const championMap = new Map(champions.map((u) => [unitMatchKey(u.name), u]));
  const traitMap = new Map(traitRoster.map((s) => [s.name, s]));
  ...
  return {
    ...
    favoriteComp,
    compFrequency,
    traitThresholdHistory: buildTraitThresholdHistory(sortedSlice, championMap, traitMap),
