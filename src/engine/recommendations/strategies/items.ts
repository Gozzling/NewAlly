...
export function itemRecommendations(input: RecommendationEngineInput, nowMs = Date.now()): AllyRecommendation[] {
  ...
  const t = RECOMMENDATION_THRESHOLDS.items;
  ...
  if (labels.length > 0) {
    ...
    out.push({
      ...
      confidence: clamp01(
        t.craftConfidenceBase +
          t.craftConfidencePerCraftable *
            Math.min(labels.length, t.craftConfidenceCraftableCap),
      ),
      risk: "medium",
      urgency:
        signals.itemMissingCount > t.missingCountUrgencyMedium ? "medium" : "low",
      reasoning: [
    ...
    out.push({
      ...
      confidence: t.partialSlamMinConfidence,
      risk: "low",
      urgency: "low",
      reasoning: [
