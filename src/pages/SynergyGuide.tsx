...
import { buildTraitGuideEntries, type TraitGuideEntry } from '@/lib/tftStaticMappers'
...
export function SynergyGuide({ query, setQuery, typeFilter, setTypeFilter, onSynergySelect }: SynergyGuideProps) {
  ...
  const traits = useMemo(() => buildTraitGuideEntries(tft), [tft])
  const [selectedSynergy, setSelectedSynergy] = useState<TraitGuideEntry | null>(null)
    ...
    let list = query
      ? traits.filter(
          (s) =>
            ...
            s.apiName.toLowerCase().includes(query.toLowerCase()) ||
            s.bestUnits.some((u) => u.toLowerCase().includes(query.toLowerCase())),
        )
      : traits
  ...
  const handleSynergyClick = (synergy: TraitGuideEntry) => {
  ...
  return (
    <div className="flex h-screen" style={{ animation: 'pageEnter 0.4s cubic-bezier(0.25, 1, 0.5, 1)' }}>
      ...
      <div className="flex-1 overflow-y-auto" style={{
        ...
        <TFTStaticDataBanner meta={tft.meta} count={filtered.length} label="traits" />
...
function SynergyCard({ synergy, index, onClick }: { synergy: TraitGuideEntry; index: number; onClick: () => void }) {
...
function SynergyDetail({ synergy, onBack }: { synergy: TraitGuideEntry; onBack: () => void }) {
  const typeColors = TYPE_COLORS[synergy.type] ?? TYPE_COLORS.hybrid
