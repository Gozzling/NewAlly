    ...
    <div className="p-4 space-y-3 select-none min-w-[560px]">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div className="flex items-center gap-2">
          ...
          <select value={sid} onChange={e => setSid(e.target.value)} className="text-xs bg-[#1f1f1f] border border-[#2a2a2a] text-white rounded px-2 py-1">
      ...
      {/* Board */}
      <div className="relative bg-[#1a1510] rounded-xl border border-[#2a2218] shadow-inner p-3 inline-block">
            ...
            for (let c = 0; c < cnt; c++) {
              ...
              els.push(
                <g key={`${r}-${c}`} transform={`translate(${x},${y})`}>
                  <polygon points={hp(HW, HH)} fill={u ? CB[unit?.cost || 1] : '#2a2218'} stroke={u ? '#5a4a38' : '#4a3f2a'} strokeWidth={2}
        ...
        <div className="mt-3 flex gap-1.5 justify-center">
          {bench.map((u, i) => {
            const unit = u ? UNITS.find(x => x.name === u) : null
            return (
              <div key={i} onClick={() => { if (u) remove('bench', i); else setPk({ t: 'bench', i }) }}
